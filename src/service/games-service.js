...
export const saveToFavs = game => new Promise(async (resolve) => {
  const favGames = localStorage.getItem('fav-games') || '[]';
  const data = JSON.parse(favGames);
  data.push(game);
...
export const removeFromFavs = game => new Promise(async (resolve) => {
  const favGames = localStorage.getItem('fav-games') || '[]';
  const data = JSON.parse(favGames);
...
export const getAllFavs = sizeLimit => new Promise(async (resolve) => {
  const favGames = localStorage.getItem('fav-games') || '[]';
  const data = JSON.parse(favGames);
  let gameList = [...data];
  gameList = gameList.sort(() => 0.5 - Math.random()).slice(0, sizeLimit);
...
export const searchInFavs = (searchTerm, sizeLimit) => new Promise(async (resolve) => {
  const favGames = localStorage.getItem('fav-games') || '[]';
  const data = JSON.parse(favGames);
  let gameList = [...data];
  gameList = gameList.filter(
