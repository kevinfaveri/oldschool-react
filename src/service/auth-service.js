export const registerUser = registerForm => new Promise((resolve) => {
...
export const isUserLogged = () => {
  ...
  if (
    userInfo
    && userInfo.lastAccess
    && (new Date().getTime() - new Date(userInfo.lastAccess).getTime()) / 1000 < 300
  ) {
    return true;
  }
  return false;
...
export const loginUser = loginForm => new Promise((resolve) => {
  setTimeout(() => {
    const userInfo = JSON.parse(localStorage.getItem('userInfo'));
    if (
      userInfo
